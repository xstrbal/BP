...
int BPU_mecsInitCtx(BPU_T_Mecs_Ctx **ctx, const BPU_T_UN_Mecs_Params *params, const BPU_T_EN_Mecs_Types type) {
    ...
    case BPU_EN_MECS_BASIC_QCLDPC1:
#ifdef BPU_CONF_ENCRYPTION
        ctx_p->_encrypt = BPU_mecsBasicEncrypt;
#endif
#ifdef BPU_CONF_DECRYPTION
...
#endif
#ifdef BPU_CONF_KEY_GEN
        ctx_p->_genKeyPair = BPU_mecsQcldpcGenKeys;
#endif
        rc = BPU_codeInitCtx(&ctx_p->code_ctx, (BPU_T_UN_Code_Params *)params, BPU_EN_CODE_QCLDPC1);
        if (rc) {
            return rc;
        }
        ctx_p->pt_len = ctx_p->code_ctx->msg_len;
	...
	case BPU_EN_MECS_BASIC_QCLDPC2:
#ifdef BPU_CONF_ENCRYPTION
		ctx_p->_encrypt = BPU_mecsBasicEncrypt;
#endif
#ifdef BPU_CONF_DECRYPTION
...
#endif
		rc = BPU_codeInitCtx(&ctx_p->code_ctx, (BPU_T_UN_Code_Params *)params, BPU_EN_CODE_QCLDPC2);
		if (rc) {
			return rc;
		}
		ctx_p->pt_len = ctx_p->code_ctx->msg_len;
...
int BPU_mecsInitParamsQcldpc(BPU_T_UN_Mecs_Params *params, const uint16_t m, const uint16_t n0, const uint16_t w, const uint16_t t) {
    return BPU_codeInitParamsQcldpc((BPU_T_UN_Code_Params *)params, m, n0, w, t);
}

void BPU_mecsFreeParamsQcldpc(BPU_T_UN_Mecs_Params *params) {
    BPU_codeFreeParamsQcldpc((BPU_T_UN_Code_Params *)params);
}
