...
#endif
	BPU_EN_MECS_BASIC_QCLDPC1, ///< qc-ldpc mecs, with zero blocks in S and Q
	BPU_EN_MECS_BASIC_QCLDPC2 ///< qc-ldpc mecs, without zero blocks in S and Q
}BPU_T_EN_Mecs_Types;
...
/**
 * Wrapper for param initialization. After work you have to free memory using call BPU_mecsFreeParamsQcldpc
 * @param  params pointer to structure
 * @param  m      length of a generating polynomial
 * @param  n0     number of cyclic matrices H
 * @param  w      row/column Hamming weight of parity-check matrix H
 * @param  t      Hamming weight of error vector
 * @return        [description]
 */
