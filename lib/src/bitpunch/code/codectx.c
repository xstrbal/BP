...
int BPU_codeInitCtx(BPU_T_Code_Ctx **ctx, const BPU_T_UN_Code_Params *params, const BPU_T_EN_Code_Types type) {
	...
	*ctx = (BPU_T_Code_Ctx *) calloc(1, sizeof(BPU_T_Code_Ctx));
	...
	case BPU_EN_CODE_QCMDPC:
		...
		break;
	case BPU_EN_CODE_QCLDPC1:
		ctx_p->code_spec->qcldpc = (BPU_T_Qcldpc_Spec *) calloc(1, sizeof(BPU_T_Qcldpc_Spec));
#ifdef BPU_CONF_ENCRYPTION
		ctx_p->_encode = BPU_mecsQcldpcEncode1;
		
#endif
#ifdef BPU_CONF_DECRYPTION
		ctx_p->_decode = BPU_mecsQcldpcDecrypt1;
		
#endif
		if (!ctx_p->code_spec->qcldpc) {
			BPU_printError("Can not malloc BPU_T_QCLDPC_Spec");

			return BPU_EC_MALLOC_ERROR;
		}
		ctx_p->code_spec->qcldpc->m = params->qcldpc->m;
		ctx_p->code_spec->qcldpc->n0 = params->qcldpc->n0;
		ctx_p->code_spec->qcldpc->w = params->qcldpc->w;		
		ctx_p->code_len = params->qcldpc->m * params->qcldpc->n0;
		ctx_p->msg_len = ctx_p->code_len - params->qcldpc->m;
		ctx_p->t = params->qcldpc->t;

		break;

	case BPU_EN_CODE_QCLDPC2:
		ctx_p->code_spec->qcldpc = (BPU_T_Qcldpc_Spec *)calloc(1, sizeof(BPU_T_Qcldpc_Spec));
#ifdef BPU_CONF_ENCRYPTION
		ctx_p->_encode = BPU_mecsQcldpcEncode2;

#endif
#ifdef BPU_CONF_DECRYPTION
		ctx_p->_decode = BPU_mecsQcldpcDecrypt2;
...
void BPU_codeFreeCtx(BPU_T_Code_Ctx **ctx) {
	...
	case BPU_EN_CODE_QCMDPC:
		...
		break;
	case BPU_EN_CODE_QCLDPC1:
		BPU_qcldpcFreeSpec(ctx_p->code_spec->qcldpc);
		break;
	case BPU_EN_CODE_QCLDPC2:
		BPU_qcldpcFreeSpec(ctx_p->code_spec->qcldpc);
		break;
...
int BPU_codeInitParamsQcldpc(BPU_T_UN_Code_Params *params, const uint16_t m, const uint16_t n0, const uint16_t w, const uint16_t t) {
	return BPU_qcldpcInitParams(&params->qcldpc, m, n0, w, t);
}

void BPU_codeFreeParamsQcldpc(BPU_T_UN_Code_Params *params) {
	BPU_qcldpcFreeParams(&params->qcldpc);
}
